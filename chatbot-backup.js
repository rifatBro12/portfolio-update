class MahmudulAssistant {
    ...
    async generateResponse(userMessage) {
        ...
        // Contact information
        if (this.containsAny(message, ['contact', 'email', 'reach', 'hire', 'work together', 'phone', 'call'])) {
            const contact = this.knowledgeBase.contact;
        ...
        if (this.containsAny(message, ['ai', 'artificial intelligence', 'machine learning', 'ml', 'chatbot', 'voice recognition'])) {
            return `Mahmudul is passionate about AI! 🧠 He's built AI chatbots, voice recognition systems, and sentiment analysis tools. His Medical Chatbot helps users get health guidance, and his Voice Ordering App revolutionizes customer interactions. He combines his full-stack development skills with AI to create intelligent solutions that solve real problems!`;
        }

        // Web development
        if (this.containsAny(message, ['web development', 'frontend', 'backend', 'django', 'react', 'javascript'])) {
            return `Mahmudul is a full-stack web development expert! 🌐 He's proficient in frontend technologies like JavaScript, HTML, CSS, and React, as well as backend development with Django and Python. He creates beautiful, responsive websites like this portfolio, and integrates them with powerful backend systems. Need a website or web application? He's your guy!`;
        }
        ...
        if (this.containsAny(message, ['price', 'cost', 'rate', 'budget', 'charges'])) {
            return `Great question! 💰 Mahmudul believes in fair, competitive pricing based on project scope and complexity. He offers both hourly rates and project-based pricing. For AI/ML projects, web development, or custom solutions, he'd be happy to discuss your specific needs and provide a detailed quote. Contact him at ${this.knowledgeBase.personal.email} for a personalized estimate!`;
        }
        ...
        return `${contextualResponse} Mahmudul specializes in ${this.knowledgeBase.skills.technologies.join(', ')}, has ${this.knowledgeBase.experience.years} of experience, and has created projects like ${this.knowledgeBase.projects[0].name} and ${this.knowledgeBase.projects[1].name}. What specific aspect interests you most? 🤔`;
    }
